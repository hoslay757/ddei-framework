...
request.interceptors.request.use(
  config => {
    if (config.url.includes('/refresh/token' || config.url.includes('/user/login') || config.url.includes('/user/register'))) {
      return config
    }
    // 验证失效时间
    ...
    if (stayMin < 30 && stayMin > 0) {
      // 小于30分钟刷新token
      request.post(config.baseURL + '/v1/refresh/token?refreshToken=' + Cookies.get('refreshToken')).then(res => {
